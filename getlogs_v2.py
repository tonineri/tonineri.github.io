...
if("json" in args['format']):
    ...
    for log in response['hits']['hits']:
        for field in args['fields']: ##Compounds dictionaries inside of dictionaries into one dictionary so writerow works as intended.
            # This code assumes that no field exists in two sub-dictionaries, but 'pod' is in both kube.pod and properties.pod
            # That causes a problem because both values for pod are added to valueStore, but there is only one field
            # We end up with more values in the valueStore than there are fields, and they become misaligned by the time we later
            # combine them with the zip function.
            # My proposed solution is that when we have found a value for a field, set a boolean variable, which I have called
            # foundValueForThisFieldAlready but please feel free to rename it to something shorter, to true.
            # Then, if we later find more values for the same field, we should not add them to the valueStore array
                            ...
                            if not field in item:
                                if type(log[arg][item]) == dict:
                                    for subitem in log[arg][item].keys():
                                        if subitem in field:
                                            if not foundValueForThisFieldAlready:
                                                valueStore.append(log[arg][item][subitem])
                                                foundValueForThisFieldAlready=True
                            else:
                                if not foundValueForThisFieldAlready:
